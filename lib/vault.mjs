...
export class Vault {
  ...
  static async verifyMultikey(jws, ...tags) {
    ...
    JSON.parse(jws).signatures.forEach(sub => {
      let {iss, act} = JOSE.decodeProtectedHeader(sub);
      team = combineHeader('iss', team, iss);
      member = combineHeader('act', member, act);
    });
    if (team && tags[0] !== team) return; // first tag must match iss if present
    if (member && -1 === tags.indexOf(member)) tags.push(member); // automatically add act if needed
  ...
  static async createKeys() { // Promise a new tag and private keys, and store the encrypting key.
    ...
    await this.store('EncryptionKey', tag, exportedEncryptingKey, signature);
  ...
  static setWrapped(tag, wrapped, signature) { // Promise to persist the wrapped key appropriately for this class.
    return this.store(this.collection, tag, wrapped, signature);
  }
  ...
  async destroy(options = {}) { // Terminates this vault and associated storage, and same for OWNED recursiveMembers if asked.
    ...
    await this.constructor.store('EncryptionKey', tag, content, signature);
    await this.constructor.store(this.constructor.collection, tag, content, signature);
    this.constructor.clear(tag);
  ...
  static localKey(collectionName, tag) {
    // EncryptionKey must always be stored in the cloud, so that any of your devices can rewrap your indvidual team with all your devices.
    return (collectionName === DeviceVault.collection) && `${collectionName}/${tag}`;
  }
  ...
  static async store(collectionName, tag, string, signature) {
    let local = this.localKey(collectionName, tag);
    if (local) {
      if (string) set(local, signature);
      else del(local);
      return;
    }
  ...
  static async retrieve(collectionName, tag) {
    let local = this.localKey(collectionName, tag),
	jwsPromise = local ? get(local) : Vault.Storage.retrieve(collectionName, tag),
	jws = await jwsPromise,
	payload = this.fixmePayload(jws);
    return payload;
  }
