...
export class Vault {
  ...
  static async verify(signature, tags, options) {
	...
	result = await MultiKrypto.verify(verifyingKey, signature, options);
      } // else result is undefined, deliberately
    } else {
      let key = {}, team, member;
      JSON.parse(signature).signatures.forEach(sub => {
	...
	if (kid && !tags.includes(kid)) tags.push(kid);
      ...
      result = await MultiKrypto.verify(key, signature);
  ...
  static async retrieve(collectionName, tag) {
    ...
    return (await MultiKrypto.verify(key, signature)).text;
  }
