...
export class Vault {
  ...
  static async sign(message, {tags = [], team:iss, member:act, time:iat}) {
    if (iss && !act) { // Supply the value
      let teamVault = await Vault.ensure(iss),
	  memberTags = teamVault.memberTags,
	  cachedMember = memberTags.find(tag => Vault.vaults[tag]);
  ...
  async destroy(options = {}) { // Terminates this vault and associated storage, and same for OWNED recursiveMembers if asked.
	...
	signature = await this.constructor.storageSignature(content, tag);
...
export class SecretVault extends Vault { // Keys are encrypted based on a symmetric secret.
  static storageSignature(content, tag) {
    return this.sign(content, {tags: [tag]});
  }
...
export class TeamVault extends Vault { // A Vault corresponding to a team of which the current user is a member (if getTag()).
  ...
  static storageSignature(content, tag) {
    return this.sign(content, {team: tag});
  }
