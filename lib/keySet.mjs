...
export class KeySet {
  ...
  static async ensure(tag) { // Promise to resolve to a valid vault, else reject.
    ...
    return vault.unwrap(vault.cached).then(
      unwrapped => Object.assign(vault, unwrapped),
      cause => {
	this.clear(vault.tag)
	return error(tag => `You do not have access to the private key for ${tag}.`, vault.tag, cause);
      });
  }
