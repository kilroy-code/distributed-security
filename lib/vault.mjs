...
export class Vault {
  ...
  static async sign(message, {tags = [], team:iss, member:act, time:iat,
			      // These two can be passed in so that this can be used in signing our own keys
			      // for storage as the vault is beging created, at which point, we cannot
			      // call Vault.ensure!
    ...
    if (!iss && !act && !iat && tags.length === 1) {
      ...
      return MultiKrypto.sign(signingKey, message);
    }
  ...
  static async persist(tag, keys, wrappingData) { // Promise to wrap a set of keys for the wrappingData members, and persist by tag.
	...
	signature = await this.signForStorage({message: wrapped, tag, signingKey, memberTags: wrappingData});
