...
export class Vault {
  ...
  // FIXME: is this the right approach? Don't we have to do that with within the MultiKrypto.wrap?
  // FIXME: When we consistently use JWK, we can remove the conditionals. i.e., always treat a json.
  static asString(value) { // Return a string representing value that we can unpickle with fromString
    return value;
    // if ('string' === typeof(value)) return value;
    // return JSON.stringify(value);
  }
  static fromString(string) { // Unpickle a a string that had passed through asString
    return string;
    // if (string.startsWith('{') && string.endsWith('}')) return JSON.parse(string);
    // return string;
  }
  ...
  static verifyingKey(tag) { // Promise the ordinary singular public key corresponding to the signing key, directly from the tag without reference to storage.
    return MultiKrypto.importRaw(tag, 'verify').catch(_ => unavailable(tag));
  }
  static async encryptingKey(tag) { // Promise the ordinary singular public key corresponding to the decryption key, which depends on public storage.
    ...
    let retrievedPublicKey = await Vault.Storage.retrieve('EncryptionKey', tag),
	exportedPublicKey = this.fromString(retrievedPublicKey);
  ...
  static async createKeys() { // Promise a new tag and private keys, and store the encrypting key.
    ...
    await Vault.Storage.store('EncryptionKey', tag, this.asString(exportedEncryptingKey), signature);
  ...
  static async getWrapped(tag) {
    let string = await Vault.Storage.retrieve(this.collection, tag);
    if (!string) return;
...
export class SecretVault extends Vault { // Keys are encrypted based on a symmetric secret.
  static async wrap(keys, wrappingData = '') {
	...
	iv = 'fixmeIgnore', //MultiKrypto.arrayBuffer2base64string(MultiKrypto.iv()),
	secret = {[wrappingData]: await this.getSecret(tag, /*iv,*/ wrappingData)},
  ...
  async unwrap(wrapped) {
    ...
    let
    secret = {[prompt]: await this.constructor.getSecret(this.tag, /*iv, */prompt)},
