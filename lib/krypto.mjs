...
const Krypto = {
  ...
  async sign(privateKey, message) {
    let header = {alg: signingAlgorithm},
  ...
  async verify(publicKey, signature) {
    ...
    return this.recoverDataFromContentType(result);
  },
  ...
  async encrypt(key, message) {
	...
	alg = this.isSymmetric(key) ? 'dir' : encryptingAlgorithm,
	secret = this.keySecret(key);
    return await new JOSE.CompactEncrypt(inputBuffer).setProtectedHeader({alg, enc: symmetricAlgorithm }).encrypt(secret);
  },
  ...
  // We generate JWK, and nested JWK for multi-key sets, and then make json only at the end.
  // So we don't need something here that generates a string.
  // However, it is handy to be able to test that export/import is working, and that the size is as expected.
  // So these two only get used in unit tests.
  async exportKey(key) {
    return JSON.stringify(await this.exportJWK(key));
  },
  importKey(string, use) {
    return this.importJWK(JSON.parse(string));
  },
  ...
  async unwrapKey(wrappedKey, unwrappingKey) {
    ...
    return this.importJWK(JSON.parse(decrypted));
  }
}
