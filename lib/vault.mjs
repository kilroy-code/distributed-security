...
export class Vault {
  ...
  static async sign(message, {tags = [], team:iss, member:act, time:iat, memberTags, signingKey}) { // fixme memberTags,
    ...
    if (!iss && !act && !iat && tags.length === 1) {
      if (signingKey) return MultiKrypto.sign(signingKey, message)
      let vault = await Vault.ensure(tags[0]);
      return vault.sign(message);
    }
    let baseHeader = {iss, act, iat},
  ...
  static async createKeys(memberTags) { // Promise a new tag and private keys, and store the encrypting key.
	...
	signature = await this.signForStorage({message: exportedEncryptingKey, tag, signingKey, memberTags});
  ...
  static async create(wrappingData) { // Create a persisted Vault of the correct type, promising the newly created tag.
    let keys = await this.createKeys(wrappingData),
	{tag} = keys;
  ...
  static async persist(tag, keys, wrappingData) { // Promise to wrap a set of keys for the wrappingData members, and persist by tag.
	...
	isTeam = (this.name === 'TeamVault'),
	signature = isTeam
	? await this.signForStorage({message: wrapped, tag, signingKey, memberTags: wrappingData})
	: await this.signForStorage({message: wrapped, tag, signingKey, memberTags: wrappingData});
    //await this.signForStorage({message: wrapped, signingKey, tags: [tag], team: tag, memberTags: wrappingData});
  ...
  async destroy(options = {}) { // Terminates this vault and associated storage, and same for OWNED recursiveMembers if asked.
	...
	signature = await this.constructor.signForStorage({message: content, tag, memberTags, signingKey});
  ...
  static async retrieve(collectionName, tag) {
    ...
    if (!signature.startsWith('{')) {
      // Non-teams are signed with a compact signature, for which we have all the info to verify directly.
      let key = await Vault.verifyingKey(tag),
	  verified = await MultiKrypto.verify(key, signature);
      return verified?.text;
    }
    // Team data has an auditable signature, but doing that audit requires getting to the contents.
    let jws = JSON.parse(signature),
	buffer = JOSE.base64url.decode(jws.payload);
...
export class SecretVault extends Vault { // Keys are encrypted based on a symmetric secret.
  static signForStorage({message, tag, signingKey}) {
    ...
    return this.sign(message, {tags: [tag], signingKey});
  }
