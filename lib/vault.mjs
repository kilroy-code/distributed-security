...
export class Vault {
  ...
  static async verify(signature, tags, options) {
	...
	memberTag = options.member ?? result?.protectedHeader.act,
	notBefore = options.notBefore ?? (result?.protectedHeader.iss && 'team');
    if (!result) return;
    if (memberTag) {
    ...
    if (notBefore && (result.protectedHeader.iat < notBefore)) return;
