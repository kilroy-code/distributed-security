...
export class Vault {
  ...
  static localKey(collectionName, tag) {
    return (collectionName === DeviceVault.collectionName) && `${collectionName}/${ownerTag}`;
  }
  static store(collectionName, tag, string, signature) {
    ...
    return Vault.Storage.store(collectionName, tag, string, signature);
  }
  static retrieve(collectionName, tag) {
    let local = this.localKey(collectionName, tag);
    if (local) return localStorage.getItem(local);
    return  Vault.Storage.retrieve(collectionName, tag);
  }
  ...
  static async isCurrentMember(memberTag, teamTag) {
    // Promise to answer whether memberTag is currently a member of teamTag.
	...
	teamJWE = teamData ? JSON.parse(teamData) : {};
    return teamJWE.recipients.find(member => member.header.kid === memberTag);
  }
}
