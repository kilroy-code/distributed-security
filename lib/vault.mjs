...
// Can't hurt to ask...
navigator.storage.persist().then(persistent => console.log(`Device keys ${persistent ? 'will not' : 'may'} be cleared without explicit user action.`));
...
onmessage = event => {
  ...
  const entryModuleName = 'entry!'+cloudName,
        api = { // jsonrpc requests from the client are handled by these, which answers them by making requests to the worker.
          ...
          wipeDeviceKeys() { return postWorker('wipeDeviceKeys'); }
        },
        postClient = dispatch({
