...
export class TeamVault extends Vault { // A Vault corresponding to a team of which the current user is a member (if getTag()).
  ...
  static async wrap(keys, members) { // Create a new publically persisted TeamVault with the specified member tags, promising the tag.
    // This is used by persist, which in turn is used create and changeMembership.
    let {decryptingKey, signingKey, tag} = keys,
  ...
  async unwrap(wrapped) {
	...
	vault = await Promise.any(nonRecoveryTags.map(memberTag => Vault.ensure(memberTag)))
	...
	decrypted = await vault.decryptMultikey(wrapped);
