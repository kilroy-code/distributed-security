...
onmessage = event => {
  console.log('***', self.name, 'got', event.data, event.origin, event.source, event.ports);
  ...
  const url = import.meta.url,
        vaultOrigin = new URL('worker-bundle.mjs?v=53', url),
        ...
        hostAPI = { // jsonrpc request from the worker are handled by these, which answers them by make requests to the client.
          ...
          ready(label) {
            console.log('*** worker is ready', label);
            event.ports[0].start();
            console.log('*** started port2', event.ports[0]);
            postClient('ready', label);
            console.log('*** posted ready');
          }
        },
        postWorker = dispatch({
          target: worker,
