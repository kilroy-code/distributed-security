...
export class Vault {
  ...
  static async encryptingKey(tag) { // Promise the ordinary singular public key corresponding to the decryption key, which depend on public storage.
    ...
    if (!exportedPublicKey) return Promise.reject(new Error(`The encryption key for ${tag} is not available.`));
  ...
  static async ensure(tag) { // Promise to resolve to a valid vault, else reject.
    ...
    if (!stored) {
      ...
      return Promise.reject(new Error(`Key ${tag} not found.`));
    }
    try {
      ...
      delete this.vaults[tag];
      return Promise.reject(new Error(`You do not have access to the private key corresponding to ${tag}.`, {cause}));
    }
    return vault;
  }
