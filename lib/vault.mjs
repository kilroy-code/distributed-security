...
export class Vault {
  ...
  static async sign(message, {tags = [], team:iss, member:act, time:iat, memberTags, signingKey}) { // fixme memberTags, 
    ...
    if (iss && !tags.includes(iss)) tags.push(iss);
    if (act && !tags.includes(act)) tags.push(act);
  ...
  static async createKeys(memberTags) { // Promise a new tag and private keys, and store the encrypting key.
	...
	signature = await this.signForStorage({message: exportedEncryptingKey, signingKey, tags: [tag], team: tag, memberTags});
