...
const Security = {
  ...
  async sign(message, ...tags) {
    ...
    if (tags[0]?.team) {
      ...
      iss = team;
      act = member;
      if (team && !member) { // Supply the value
	let teamVault = await Vault.ensure(team),
	    memberTags = teamVault.memberTags,
	    cachedMember = memberTags.find(tag => Vault.vaults[tag]);
	act = member = cachedMember || Promise.any(memberTags.map(tag => Vault.ensure(tag))).then(vault => vault.tag);
      }
  ...
  async verify(signature, ...tags) { // Promise true if signature was made by tag, else false.
    // If iss and act are specified, automatically add these to tags.
    if (tags.length === 1) {
