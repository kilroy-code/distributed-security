...
// sig = await new JOSE.SignJWT().setSubject("foo").setProtectedHeader({alg:'ES384'}).sign(sk.privateKey)
// (await JOSE.jwtVerify(sig, sk.publicKey)).payload.sub
// sig = await new JOSE.FlattenedSign(new TextEncoder().encode('foo')).setProtectedHeader({alg:'ES384'}).sign(sk.privateKey) // Or CompactSign
// sig = await new JOSE.GeneralSign(new TextEncoder().encode('foo')).addSignature(sk.privateKey).setProtectedHeader({alg:'ES384'}).sign()
// new TextDecoder().decode((await JOSE.generalVerify(sig, sk.publicKey)).payload)  // or compactVerify or flattenedVerify
//
// ek = await JOSE.generateKeyPair('RSA-OAEP-256', {extractable: true})
// enc = await new JOSE.FlattenedEncrypt(new TextEncoder().encode('foo')).setProtectedHeader({alg: 'RSA-OAEP-256', enc: 'A256GCM' }).encrypt(ek.publicKey) // or CompactEncrypt
//   for symmetric secret, specify alg:'dir'.
// new TextDecoder().decode((await JOSE.flattenedDecrypt(enc, ek.privateKey)).plaintext) // or compactDecrypt
// enc = await new JOSE.GeneralEncrypt(new TextEncoder().encode('foo')).setProtectedHeader({alg: 'RSA-OAEP-256', enc: 'A256GCM' }).addRecipient(ek.publicKey).encrypt() // with additional addRecipent() as needed
// new TextDecoder().decode((await JOSE.generalDecrypt(enc, ek.privateKey)).plaintext)
// enc = await new JOSE.GeneralEncrypt(new TextEncoder().encode('foo')).setProtectedHeader({alg: 'RSA-OAEP-256', enc: 'A256GCM' })
//         .addRecipient(ek.publicKey).setUnprotectedHeader({kid: 'foo'})
//         .addRecipient(ek2.publicKey).setUnprotectedHeader({kid: 'bar'})
//         .encrypt()
...
const Krypto = {
  ...
  // and currently: signingAlgorithm, exportFormat
  exportable: true,
  ...
  generateSigningKey() {
    return JOSE.generateKeyPair('ES384', {extractable: this.exportable});
  ...
  async generateSymmetricKey(text, {salt, iv = this.iv()} = {}) {
    ...
    if (text) {
      ...
      secret = await JOSE.generateSecret('A256GCMKW', {extractable: this.exportable});
  ...
  generateEncryptingKey() {
    return JOSE.generateKeyPair('RSA-OAEP-256', {extractable: this.exportable});
