...
const MultiKrypto = {
  ...
  recognizedHeaders: ['type', 'iss', 'act', 'iat', 'cty'],
  ...
  async verify(key, signature, recognizedHeaders = this.recognizedHeaders) {
	...
	kids = Object.keys(key).filter(name => !recognizedHeaders.includes(name)),
	...
	result = {protectedHeader: {}, unprotectedHeader: {}, payload: first.payload, signers},
