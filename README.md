...
> This is used by Distributed-Security itself in protecting the cloud storage that holds encrypted keys. The encrypted keys (JWE of a JWK) are signed as `sign(JWE, {team: tag, time})` so that "iss", "act", and "iat" headers are specified. The cloud `store()` operation verifies this by `verify(JWS, {team: tag, notBefore: "team"})`. See below.
## Application Use
