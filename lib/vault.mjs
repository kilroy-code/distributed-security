...
onmessage = event => {
  if ((event.source !== parent) || (event.origin !== new URL(document.referrer).origin))
    return alert(`${self.name} got ${event.data} from ${event.origin}.`);
  onmessage = null;
  ...
  const url = import.meta.url,
        vaultOrigin = new URL('worker-bundle.mjs?v=54', url),
